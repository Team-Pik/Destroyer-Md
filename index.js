...
async function startHisoka() {
    ...
    hisoka.ev.on('messages.upsert', async chatUpdate => {
        ...
        require("./destroyer")(hisoka, m, chatUpdate, store)
        } catch (err) {
            console.log(err)
        }
    })
     ...
     * @returns
     */
    hisoka.send5ButGif = async (jid , text = '' , footer = '', but = [], options = {}) =>{
    let message = await prepareWAMessageMedia({ video: fs.readFileSync('./image/riych.mp4'), gifPlayback: true }, { upload: hisoka.waUploadToServer })
